...
class DocumentationBuildTests(unittest.TestCase):
    ...
    def test_landing_page_identifies_current_and_links_older_release(self) -> None:
        ...
        site = self.root / "site"
        (site / "1.9.1").mkdir(parents=True)
        (site / "1.9.1" / "index.html").write_text("current", encoding="utf-8")
        (site / "1.9.0").mkdir()
        (site / "1.9.0" / "index.html").write_text("old", encoding="utf-8")
        write_site_index(site, versions)
        page = (site / "index.html").read_text(encoding="utf-8")
        self.assertIn("Current release: 1.9.1", page)
        self.assertIn('href="1.9.0/index.html"', page)

    def test_offline_archive_is_deterministic_and_self_identifying(self) -> None:
        site = self.root / "site"
        site.mkdir()
        (site / "index.html").write_text("release 1.9.1", encoding="utf-8")
        (site / "release.json").write_text(
            '{"release":"1.9.1"}\n', encoding="utf-8"
        )
        first = self.root / "first.zip"
        second = self.root / "second.zip"
        first_digest = write_offline_archive(site, first, "1.9.1")
        second_digest = write_offline_archive(site, second, "1.9.1")
        self.assertEqual(first_digest, second_digest)
        self.assertEqual(hashlib.sha256(first.read_bytes()).hexdigest(), first_digest)
        with zipfile.ZipFile(first) as archive:
            self.assertIn(
                "mathlib-fp-docs-1.9.1/release.json", archive.namelist()
            )
