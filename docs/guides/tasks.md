...
([`RECIPES.md`](../start/recipes.md)) provide the same route in a shorter task-first
form.

If a task is not supported by a documented entry point, it is marked
`gap — roadmap candidate`; the capability inventory
([`CAPABILITIES.md`](../reference/capabilities.md)) and the post-2.0 capability programme in
[`ROADMAP.md`](../project/roadmap.md#post-20-capability-programme) track those families.
...
| European call or put price | `FinanceLib.Interest` | `TFinanceKit.BlackScholes(Spot, Strike, Rate, Vol, T, OptionType)` | [option pricing section](domains/finance.md#option-pricing-black-scholes) |
...
| Carnot efficiency of an ideal heat engine | `EngineeringLib.Thermodynamics` | `TThermodynamicsKit.CarnotEfficiency(HotK, ColdK)` | [quick-start program](domains/engineering.md#engineeringlibthermodynamics-tthermodynamicskit) |
