...
New code should copy and run the [typed double-real solve](typed-dense-matrices.md#60-second-solve),
...
| --- | --- | --- |
| Square dense solve | `Solve` / `SolveWithInfo` | [Dense solver choice](dense-linear-algebra.md#choose-a-dense-solver) |
| Tall least squares | `LeastSquares` | [Least-squares recipe](../../start/recipes.md#dense-least-squares) |
| Sparse positive-definite solve | `ConjugateGradient` | [Sparse solver choice](sparse-linear-algebra.md#choose-an-iterative-solver) |
| Matrix multiplication | `Multiply` | [Typed entry-point choice](typed-dense-matrices.md#choose-an-entry-point) |
...
[Typed dense decompositions and solvers](dense-linear-algebra.md). The
large-problem selection, storage, residual, and reuse contracts are in
[Sparse, structured, and matrix-free linear algebra](sparse-linear-algebra.md).
The
...
New code can use the typed [`Solve(A, B)` path](typed-dense-matrices.md), which
...
[typed dense five-minute guide](typed-dense-matrices.md#60-second-solve). The
