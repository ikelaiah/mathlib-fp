...
4. Review all 21 plain compiler aliases against behavior, defaults, ownership,
   exception identity, and numerical results. Retain 17 and route the four
   pressure/velocity aliases to 1.9.7 migration/package-boundary confirmation
   without deprecating them.
5. Give all 131 compatibility rows exactly one decision: typed replacement
   plus semantic difference for matrix and pressure/velocity aliases, or
   explicit retention for the focused finance alias units.
6. Publish a machine-readable and human-readable exact proposed diff. Source,
   behavior, warning, and packaging lists are explicitly empty; documentary
   defaults are named separately.
7. Add selector, overload, generic-owner, exact-coverage, alias-review,
   common-example, convention, compatibility, and diff checks to release
   qualification.
...
| --- | --- |
| No undecided stable declaration/alias/default/ownership/classification/replacement | `api-decision-2.0.json` has an empty unresolved list; `check_api_decision.py` reconstructs every classification, resolves all domains/concerns, requires exactly one review for every plain compiler alias, and requires exactly one decision for every compatibility row. |
...
- Run `git diff --check`, inspect that `src/` has no diff, and review generated
  classification totals plus all 21 alias reviews and compatibility mappings.
